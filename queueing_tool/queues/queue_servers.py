...
class QueueServer :
    ...
    keep_data : bool (the default is False)
    ...
    data : dict
        ...
        departure times of all arrivals. The keys are the :class:`~Agent`'s unique ``issn``,
    ...
    colors : dict
        ...
        ``edge_loop``: The default color of the edge if the edge is a loop.
        ``edge_normal``: The normal color a non-loop edge.
        ``vertex_normal``: The normal fill color for a vertex. This colors the target
        vertex in the graph.
        ``vertex_pen``: The color of the pen for the of the target vertex.
        The defaults are:
    ...
    def set_nServers(self, n) :
        """Change the number of servers in the queue to ``n``, where ``n`` is positive integer. If ``n``
        is not a positive integer than an Exception is raised.
        """
    ...
    def next_event_type(self) :
        """Returns an integer representing whether the next event is an arrival (returns 1), a departure
        (returns 2) or nothing (returns 0).
        """
        if self._arrivals[0]._time < self._departures[0]._time :
            return 1
        ...
        if self._arrivals[0]._time < self._departures[0]._time :
            ...
            if self.keep_data :
                if arrival.issn not in self.data :
                    self.data[arrival.issn] = [[arrival._time, 0, 0]]
                else :
                    self.data[arrival.issn].append([arrival._time, 0, 0])
            ...
            self._local_t     = new_depart._time
            self.nDepartures += 1
            self._nTotal     -= 1
            self.nSystem     -= 1

            if self.keep_data and new_depart.issn in self.data :
                self.data[new_depart.issn][-1][2] = self._local_t

            if len(self._queue) > 0 :
                agent = self._queue.popleft()
                if self.keep_data and agent.issn in self.data :
                    self.data[agent.issn][-1][1] = self._local_t

                agent.set_departure(self.service_f(self._local_t))
                heappush(self._departures, agent)
    ...
    def __deepcopy__(self, memo) :
        ...
        new_server._local_t    = copy.copy(self._local_t)
...
class LossQueue(QueueServer) :
    ...
    qbuffer : int (optional, the default is 0)
        Specifies the length of the buffer (i.e. specifies how long the size of the queue can be).
...
class NullQueue(QueueServer) :
    ...
    A queue that is used but the ``QueueNetwork`` class to represent agents leaving the network.
    """
